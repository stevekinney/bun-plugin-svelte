...
For an app that mixes hydrated pages with pages that ship no client JavaScript at all, `'external'` cannot get component CSS to the zero-JS pages—a server compile in `'external'` mode emits no CSS at all (there is no client bundle for those pages to pull a stylesheet into). `'injected'` does work there: `render()` computes the `<style>` tag on the server and puts it directly in `head`, so the CSS reaches the page as static HTML with no client JS required to run. That makes `'injected'` the workable default for a mixed app—the same mode delivers styles into `head` for zero-JS pages and appends them at runtime for hydrated ones, without duplicating a style tag the server already sent. Reach for `'none'` plus a stylesheet built out of band only when you specifically don't want the plugin managing delivery at all, such as a component library whose consumers already own their own CSS pipeline.
